...
const createToken = (id: string) => {
  return jwt.sign({ id }, config.JWT_KEY, { expiresIn: maxDay });
};
...
export const logIn = async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.status(REQUEST_TYPE_STATUS_CODE.ok).json({ status: 'Ok', user: user._id, redirect: '/blogs' });
  } catch (err) {
...
export const signUp = async (req: Request, res: Response) => {
  try {
    ...
    const token = createToken(user._id);
    res.cookie('jwt', token, { httpOnly: true, maxAge: maxDay * 1000 });
    res.status(REQUEST_TYPE_STATUS_CODE.created).json({ status: 'Created', user: { email } });
  } catch (err) {
    const errors = handleErrors(err);
